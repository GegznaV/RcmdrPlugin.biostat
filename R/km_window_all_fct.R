    ...
    methods = list(
        ...
        setFront = function() {
            ...
            vbbox1$front(
                top       = top,
                types     = list(Numeric()),
                titles    = list(gettext_Bio("Variable (pick one or more)")),
                modes     = list("multiple"),
        ...
        checkError = function(parms) {
            if (length(parms$x) == 0) {
                errorCondition(recall  = windowScatter,
...
#'
#' \code{window_all_fct} function is a wrapper function of \code{factorize}
#' class for the R-commander menu bar.
#'
#' @rdname plot-factorize-window_all_fct
#' @keywords hplot
#' @export
window_all_fct <- function() {
