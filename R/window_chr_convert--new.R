...
window_chr_convert <- function() {
    ...
    initializeDialog(
        title = gettext_bs("Convert Text Variables into Other Classes"))

    tk_title(top, "Convert text variables")
    ...
    onOK <- function() {
        ...
        if (class(result)[1] != "try-error") {
            msg <- str_glue(
                "## Convert text (character) variables into {into} variables \n\n",
                "# New variable(s): \n",
    ...
    var_y_box <- variableListBox2(
        ...
        initialSelection = var_pos_n(dialog_values$variables, "character")
    )
    ...
    Rcmdr::radioButtons(
        ...
        buttons = c("nominal", "ordinal", "integer", "numeric", "logical"),
        values  = c("nominal", "ordinal", "integer", "numeric", "logical"),
