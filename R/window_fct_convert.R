...
window_fct_convert <- function() {
    ...
    initializeDialog(
        title = gettext_bs("Convert Factor Variables into Other Classes"))

    tk_title(top, "Convert factor variables")
    ...
    onOK <- function() {
        ...
        if (class(result)[1] != "try-error") {
            msg <- str_glue(
                "## Convert factor variables into {into} variables \n\n",
                "# New variable(s): \n",
    ...
    var_y_box <- variableListBox2(
        ...
        initialSelection = var_pos_n(dialog_values$variables, "factor_strict")
    )
