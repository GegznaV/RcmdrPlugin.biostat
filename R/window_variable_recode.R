...
window_variable_recode0 <- function() {
  ...
  # FIXME: replace Rcmdr::radioButtons
  Rcmdr::radioButtons(
    variable_type_frame,
    name          = "recode_into",
  ...
  help_menu <- function() {
    ...
    tkadd(menu_main, "command",
      label    = "Function `recode_factor()`",
      command  = open_help("recode_factor", package = "dplyr"))

    tkadd(menu_main, "command",
      label    = "Function `recode()`",
      command  = open_help("recode", package = "dplyr"))
