...
```{r logo-local, fig.align='center', out.height='30%', out.width='30%',echo=FALSE} 
 knitr::include_graphics(
   "https://raw.githubusercontent.com/GegznaV/RcmdrPlugin.biostat/master/docs/logo.png"
 ) 
``` 
...
**RcmdrPlugin.biostat** is an [**Rcmdr**](https://CRAN.R-project.org/package=Rcmdr) plug-in for the most common data wrangling, visualization and analysis tasks that uses **tidyverse** family functions as well as functions from other packages.
...
Before you begin installation or updating, make sure that only a single *R*/*RStudio* session is running and restart current *R* session (to unload the packages).
It is also recommended to close current *RStudio* project, if you use one.
...
Usually, you have to delete the indicated "*00LOCK*" directory, restart current *R* session and try to install again.



### Previous versions

If you need a previous version of the package, find section "[Previous versions of **RcmdrPlugin.biostat**]" below.
...
### Note for Mac users only {#for-mac}
...
So, first step of using *R Commander* is to make sure that *XQuartz* is opened.
...
### Load by using a script {#load-by-script}

You may load the package **RcmdrPlugin.biostat** programmatically or by using menus:
...
- Option 3: use *RStudio* and *R Commander* menus (see section "[Load by using menus]").

- **Next**, turn on *BioStat* mode (see section "[Turn on *BioStat* (green) mode]").
...
> NOTE: be aware that some plug-ins are incompatible one to the other and 
        ...
        other combination of plugins.
  
  
> NOTE: if "RcmdrPlugin.biostat" is not between the options of Plugins, it means
        that the plugin is already chosen but not included in the menus of 
        *R Commander*. This means that you should restart the *R Commander* by 
        using function `restart_commander()`{.r}.

<br>

3. When **RcmdrPlugin.biostat** is loaded, you can access its functionality through `BioStat'20` menu in the menu bar:
...
A more convenient way to use the plug-in is to turn on the *BioStat* mode.
...
# Previous versions of **RcmdrPlugin.biostat** {#install-previous-versions}
