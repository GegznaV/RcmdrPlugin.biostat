...
You may also try removing `upgrade = FALSE`{.r} and manually choose the packages to update.
...
To check if updates for **RcmdrPlugin.biostat** and associated packages are needed, package (**bio**)[https://mokymai.github.io/bio/] can be used:
...
If you need a previous version of the package, find the section "[Previous versions of **RcmdrPlugin.biostat**](#previous-versions-of-rcmdrpluginbiostat)" below.
...
c) *Option 3:* use *RStudio* and *R Commander* menus (see section "[Load by using menus](#load-by-using-menus)").

<br>

**Next**, for some of the potions above, the *BioStat* mode should be turned on by using commander (see section "[Turn on *BioStat* (green) mode](#turn-on-biostat-green-mode)").
...
### Menus in *BioStat* (green) mode {#menus-in-biostat-mode}
